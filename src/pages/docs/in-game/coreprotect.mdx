...
chat and ping either Wing or August. Rollbacks can be undone with the /co restore command, however only us two
are allowed to perform this command. All coreprotect commands are logged in a channel, so undoing an accidental rollback is easy.
